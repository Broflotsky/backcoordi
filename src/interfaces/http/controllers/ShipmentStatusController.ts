...
export const getShipmentStatusByTrackingCode = async (req: Request, res: Response) => {
  try {
...
export const getShipmentStatusHistoryByTrackingCode = async (req: Request, res: Response) => {
  try {
    const { trackingCode } = req.params;
    const userId = (req as any).user.id;
    const isAdmin = (req as any).user.role_id === 1;
